...
def smooth_fp(
    ...
    F: JaxFloatArray,
    Q: JaxFloatArray
) -> tuple[JaxFloatArray,JaxFloatArray,JaxFloatArray,JaxFloatArray]:
    ...
    def _step(
        ...
        x_s,x_prev,P_prev,Pi,Sigma = state
        z,R,Q  = carry

        #Common computed terms.
        z_pred = H @ x_init
        S_pred = jnp.linalg.inv(H @ P_prev @ H.mT + R)
        resid_pred = z - z_pred
        
        #Intermediate scan variables.
        L = F @ P_prev @ H.mT @ S_pred
        lam = Sigma @ H.mT @ S_pred
        
        #Update scan variables.
        x_smoothed = x_s + lam @ resid_pred
        x_update = F @ x_prev + L @ resid_pred
        P_update = F @ P_prev @ (F - L @ H).mT + Q
        Pi_new = Pi - Sigma @ H.mT @ lam.mT
        Sigma_new = Sigma @ (F - L @ H).mT
    ...
    return tuple([
        jnp.concatenate([v_init[jnp.newaxis,:],v_scan],axis = 0) for v_init,v_scan in zip(init_state[:4],result[:4],strict = True)
    ])


def main():

    H = GaussianSelectionTransform(slice(0, 1), 2).matrix
    x0 = jnp.array([0.0,-1.])
    ...
    process_noise = WeinerProcessNoise(1,2,intensity).covariance(jnp.array(simulation.dt)) 

    fs_break = 15
    z_to_filter,z_to_smooth = measurements[:fs_break],measurements[fs_break:]
    R_to_filter, R_to_smooth = R[:fs_break],R[fs_break:]
    Q = jnp.repeat(process_noise[jnp.newaxis,...],len(z_to_smooth),axis = 0)
    ...
    print(x_filter.shape,P_filter.shape)
    x_smoothed,x_pred,P_pred,P_smoothed = smooth_fp(z_to_smooth,R_to_smooth,x_filter.squeeze(axis = 0),P_filter.squeeze(axis = 0),H,F,Q)
    ...
    ax.plot(steps,error/error[0],lw = 1.5,color = "red",label = r"$||\hat{\varepsilon_s}||_2^2$")
