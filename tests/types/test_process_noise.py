...
class TestIntensityForms:
    ...
    def test_full_matrix_intensity(self) -> None:
        ...
        Q_tilde = jnp.array([[2.0, 0.5], [0.5, 3.0]])
...
class TestAlgebraicProperties:
    ...
    def test_qd_is_positive_definite(self, n: int, p: int, dt_val: float) -> None:
        ...
        noise = WeinerProcessNoise(n=n, p=p, intensity=np.asarray(1.0))
        Qd = noise.covariance(s(dt_val))
        jnp.linalg.cholesky(Qd)  # raises LinAlgError if not PD


class TestBatching:
    """The contract: dt is a JaxFloatArray (possibly 0-d). Output shape is
    ...
    def test_batched_intensity_and_dt_broadcast(self) -> None:
        ...
        dts = jnp.array([0.1, 0.2, 0.3])
        intensities = jnp.stack([np.eye(2), 2 * jnp.eye(2), 3 * jnp.eye(2)])
        noise = WeinerProcessNoise(n=2, p=2, intensity=intensities)
        Qd = noise.covariance(dts)
        assert Qd.shape == (3, 4, 4)
    ...
    def test_broadcast_dt_against_intensity(self) -> None:
        ...
        dts = jnp.array([[0.1], [0.2], [0.3]])  # (3, 1)
        intensities = jnp.stack([np.eye(2), 2 * jnp.eye(2)])  # (2, 2, 2)
        noise = WeinerProcessNoise(n=2, p=2, intensity=intensities)
        Qd = noise.covariance(dts)
...
class TestVanLoan:
    ...
    @pytest.mark.parametrize(
        "dt_val",
        [0.1, 1.0, 3.0, jnp.array([0.1, 1.0, 3.0]), jnp.array([[0.1, 0.2], [1.4, 2.1]])],
    )
    def test_against_weiner_process(self, n: int, p: int, dt_val: float | JaxFloatArray):
        """Test correctness of van loan against weiner process."""
        intensities = jnp.array(0.1)
