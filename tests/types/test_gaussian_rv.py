...
from pyfilter.types.random_variables import GaussianRV, JaxFloatArray
...
def covariance_1d(request):
    ...
    if request.param == "array_1d":
        ...
        return cholesky_factor(jnp.array([[4.0]]))
    elif request.param == "diagonal_1d":
        return DiagonalCovariance(jnp.array([2.0]))
...
def covariance_batched(request):
    ...
    if request.param == "array_batched":
        return jnp.array(
            [[[4.0, 1.0], [1.0, 9.0]], [[1.0, 0.5], [0.5, 1.0]]],
        )
...
def batched_matrix() -> JaxFloatArray:
    ...
    return jnp.array(
        [[[1.0, 0.0], [0.5, 2.0]], [[0.0, 1.0], [1.0, 0.0]]],
    )  # Shape (2, 2, 2)
...
class TestGaussianRVCompatibility:
    ...
    def test_check_compatible_array_success(self, grv_2d):
        key = jax.random.key(33)
        array = jax.random.uniform(jax.random.split(key)[1], shape=(1, 2))  # Broadcastable to (2,)
...
class TestGaussianRVSubtraction:
    ...
    def test_sub_grv(self, grv_2d, grv_2d_other):
        ...
        assert_allclose(res.mean, m1 - m2)
        assert_allclose(_to_array(res.covariance), c1 + c2)  # Covariances add for subtraction!
        assert isinstance(res, GaussianRV)
...
class TestGaussianRVMultiplication:
    ...
    def test_mul_1d_array(self, grv_2d, array_1d):
        ...
        if isinstance(grv_2d.covariance, jnp.ndarray):
            ...
            cov_scale = array_1d[:, jnp.newaxis] * array_1d[jnp.newaxis, :]
...
class TestGaussianRVMethods:
    ...
    def test_marginal(self, grv_batched):
        ...
        marginal_grv = grv_batched.marginal(jnp.array(indices))

        expected_mean = m[..., jnp.array(indices)]
        # Using jnp.ix_ is robust for indexing
        idx_grid = jnp.ix_(jnp.array(indices), jnp.array(indices))
    ...
    def test_conditional_batched(self, grv_batched):
        ...
        grv1 = grv_batched.marginal(jnp.array([0]))  # (2, 1)
        grv2 = grv_batched.marginal(jnp.array([1]))  # (2, 1)
