...
from pyfilter.types.random_variables import FDTYPE, GaussianRV, JaxFloatArray
...
def _to_array(cov):
    ...
    return cov if isinstance(cov, jnp.ndarray) else cov.full()
...
def covariance_1d(request):
    ...
    if request.param == "array_1d":
        return jnp.array([[4.0]], dtype=FDTYPE)
...
def covariance_2d(request):
    ...
    if request.param == "array_2d":
        cov = jnp.array([[4.0, 1.0], [1.0, 9.0]], dtype=FDTYPE)
...
def covariance_2d_other(request):
    ...
    if request.param == "array_2d_other":
        return jnp.array([[1.0, 0.0], [0.0, 2.0]], dtype=FDTYPE)
        ...
        return DiagonalCovariance(np.array([1.0, jnp.sqrt(2.0)], dtype=FDTYPE))
...
def covariance_batched(request):
    ...
    if request.param == "array_batched":
        ...
        return cholesky_factor(
            jnp.array([[[4.0, 1.0], [1.0, 9.0]], [[1.0, 0.5], [0.5, 1.0]]], dtype=FDTYPE)
        )
...
def grv_1d(covariance_1d) -> GaussianRV:
    ...
    mean = jnp.array([1.0], dtype=FDTYPE)
...
def grv_2d(covariance_2d) -> GaussianRV:
    ...
    mean = jnp.array([1.0, 2.0], dtype=FDTYPE)
...
def grv_2d_other(covariance_2d_other) -> GaussianRV:
    ...
    mean = jnp.array([-1.0, 0.5], dtype=FDTYPE)
...
def grv_batched(covariance_batched) -> GaussianRV:
    ...
    mean = jnp.array([[1.0, 2.0], [3.0, 4.0]], dtype=FDTYPE)  # Shape (2, 2)
...
@pytest.fixture
def array_1d() -> JaxFloatArray:
    """A 1D array compatible with grv_2d."""
    return jnp.array([3.0, 4.0], dtype=FDTYPE)


@pytest.fixture
def array_2d_matrix() -> JaxFloatArray:
    """A 2D matrix compatible with grv_2d."""
    return jnp.array([[1.0, 0.0], [0.5, 2.0]], dtype=FDTYPE)


@pytest.fixture
def batched_array_1d() -> JaxFloatArray:
    """A batched 1D array compatible with grv_batched."""
    return jnp.array([[1.0, 2.0], [3.0, 4.0]], dtype=FDTYPE)  # Shape (2, 2)


@pytest.fixture
def batched_matrix() -> JaxFloatArray:
...
class TestGaussianRVInitialization:
    ...
    def test_fail_mean_ndim_0(self):
        with pytest.raises(ValueError, match="Mean must have at least 1 dimension"):
            GaussianRV(np.array(1.0), jnp.array([[1.0]]))
    ...
    def test_fail_cov_not_square(self):
        with pytest.raises(
            ...
            GaussianRV(
                jnp.array([1.0, 2.0]), jnp.array([[1.0, 0.0], [0.0, 1.0], [1.0, 1.0]])
            )

    def test_fail_mean_cov_dim_mismatch(self):
        with pytest.raises(ValueError, match="Last dimension of mean .* must match"):
            GaussianRV(np.array([1.0, 2.0]), jnp.array([[1.0]]))  # mean dim 2, cov dim 1

    def test_fail_batch_dim_mismatch(self):
        mean = jnp.random.rand(3, 2)  # Batch (3,)
        cov = jnp.random.rand(4, 2, 2)  # Batch (4,)
        with pytest.raises(
    ...
    def test_broadcasting_batch_dims(self):
        ...
        mean = jnp.random.rand(1, 3, 2)  # Batch (1, 3)
        cov = jnp.random.rand(5, 1, 2, 2)  # Batch (5, 1)
...
class TestGaussianRVCompatibility:
    ...
    def test_check_compatible_array_fail(self, grv_2d):
        array = jnp.random.rand(5, 3)  # Not broadcastable to (2,)
    ...
    def test_check_compatible_array_success(self, grv_2d):
        array = jnp.random.rand(1, 2)  # Broadcastable to (2,)
...
class TestGaussianRVAddition:
    ...
    def test_add_grv(self, grv_2d, grv_2d_other):
        ...
        print(
            f"grv_2d.covariance trace: {grv_2d.covariance.trace() if hasattr(grv_2d.covariance, 'trace') else jnp.trace(grv_2d.covariance)}"
        )
...
class TestGaussianRVMultiplication:
    ...
    def test_mul_1d_array(self, grv_2d, array_1d):
        ...
        if isinstance(grv_2d.covariance, jnp.ndarray):
            ...
            cov_scale = array_1d[:, jnp.newaxis] * array_1d[np.newaxis, :]
...
class TestGaussianRVMatMul:
    ...
    def test_array_ufunc_matmul(self, grv_2d, array_2d_matrix):
        """Tests jnp.matmul(A, grv) and jnp.matmul(grv, A)"""
        m, c = grv_2d.mean, _to_array(grv_2d.covariance)
        A = array_2d_matrix
    ...
    def test_matmul_batched(self, grv_batched, batched_matrix):
        ...
        expected_mean = jnp.einsum("...ij,...j->...i", A, m)
        temp = jnp.einsum("...ij,...jk->...ik", A, c)
        expected_cov = jnp.einsum("...ij,...kj->...ik", temp, A)
...
class TestGaussianRVMethods:
    ...
    def test_marginal(self, grv_batched):
        ...
        # Using jnp.ix_ is robust for indexing
        idx_grid = jnp.ix_(indices, indices)
    ...
    def test_marginal_multiple_indices(self, grv_batched):
        ...
        # Let's use a 3D grv
        m3 = jnp.array([1, 2, 3])
        c3 = jnp.array([[4, 1, 0.5], [1, 5, 0.2], [0.5, 0.2, 6]])
    ...
    def test_joint(self, grv_2d, grv_1d):
        ...
        cross_cov = jnp.array([[0.5], [0.2]])  # Shape (n1, n2) = (2, 1)
        ...
        expected_mean = jnp.concatenate([m1, m2])
        expected_cov = jnp.block([[c11, c12], [c21, c22]])
    ...
    def test_joint_fail_cross_cov_shape(self, grv_2d, grv_1d):
        cross_cov = jnp.random.rand(2, 2)  # Wrong shape (should be 2, 1)
    ...
    def test_cross_batched(self, grv_batched, batched_matrix):
        ...
        # C @ A.T
        expected_cross = jnp.einsum("...ij,...kj->...ik", c, A)
    ...
    def test_conditional_on_mean(self, grv_2d, grv_1d):
        ...
        check_cov = (
            cond_grv.covariance
            if isinstance(cond_grv.covariance, jnp.ndarray)
    ...
    def test_conditional_on_value(self, grv_2d, grv_1d):
        """Test X1 | X2 = x2"""
        c12 = jnp.array([[0.5], [0.2]])  # (2, 1)
        x2_val = jnp.array([5.0])  # Value to condition on
    ...
    def test_conditional_batched(self, grv_batched):
        ...
        c21 = jnp.swapaxes(c12, -1, -2)

        x2_val = jnp.array([[5.0], [-1.0]])  # (2, 1)

        residual = (x2_val - m2)[..., None]  # (2, 1, 1)
        c22_inv_res = jnp.linalg.solve(c22, residual)  # (2, 1, 1)
        ...
        # C_cond = C11 - C12 @ C22_inv @ C21
        c22_inv = jnp.linalg.inv(c22)  # (2, 1, 1)
